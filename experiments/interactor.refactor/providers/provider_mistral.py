"""Mistral provider implementation.

This module implements the BaseProvider interface for Mistral's API.
"""

import logging
from typing import List

from .provider_openai import OpenAIProvider

class MistralProvider(OpenAIProvider):
    """Provider implementation for Mistral AI API.
    
    This class extends the OpenAI provider since Mistral uses an OpenAI-compatible API.
    It overrides specific behaviors that differ from the standard OpenAI implementation.
    """
    
    def _setup_clients(self):
        """Initialize Mistral clients.
        
        Sets up the clients with Mistral-specific configuration.
        """
        # Use default Mistral base URL if not provided
        if not self.base_url:
            self.base_url = "https://api.mistral.ai/v1"
        
        # Call the parent method to set up the clients
        super()._setup_clients()
    
    def supports_tools(self, model: str) -> bool:
        """Check if the model supports tool calling.
        
        Mistral's API tools support depends on the specific model.
        
        Args:
            model: Model identifier to check
            
        Returns:
            bool: True if tools are supported, False otherwise
        """
        # Known tool-supporting Mistral models
        tool_supporting_models = [
            "mistral-large", "mistral-medium", "mistral-small",
            "mistral-nemo", "nemo"
        ]
        
        # Check if the model name contains any of the supported model identifiers
        for supported_model in tool_supporting_models:
            if supported_model.lower() in model.lower():
                logging.debug(f"[TOOLS] Mistral model {model} may support tools based on name")
                return super().supports_tools(model)
        
        return False
    
    def list_models(self) -> List[str]:
        """List available models for Mistral.
        
        Returns:
            List[str]: List of available model identifiers
        """
        # Try the standard OpenAI models endpoint first
        try:
            return super().list_models()
        except Exception as e:
            logging.error(f"Failed to list Mistral models: {e}")
            
            # Fallback: return a list of known Mistral models
            return [
                "mistral-tiny",
                "mistral-small",
                "mistral-medium",
                "mistral-large",
                "mistral-nemo",
                "open-mistral-7b",
                "open-mixtral-8x7b"
            ] 